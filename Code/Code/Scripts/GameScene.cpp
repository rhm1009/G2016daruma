...
void	GameLoop(void)
{
	...
	if (GmSt.status == GameState::Start) {
		...
		if (Reset) {
			if (Blk[0].bPosition.x <= 400 || 
				Blk[0].bPosition.x >= 880) {
				if (BlkReset) {
					if (iScore >= 150) iBlkClr = 3;
					if (iScore >= 300) iBlkClr = 4;
					for (int i = 0; i < 8; i++) {
						BlkVl[i].Rdm = BlkVl[i + 1].Rdm;
						BlkVl[8].Rdm = rand() % iBlkClr + 2; // % 2 = 2block, % 3 = 3block, % 4 = 4block
					}
					BlkReset = false;
				}
				BG_Y_Pos1--;
			...
			if (Blk[0].bPosition.x <= 100 || 
				Blk[0].bPosition.x >= 1180) {
				Reset = false;
			}
		}
	}
	...
	if (GmSt.status == GameState::Over) {
		...
		if (iGmCnt < 60) {
			...
			if (OrderCnt >= Btn_Visbl)
			{
				...
				if ((WD_Size_W/2 - BtnSize_X <= Mouse.Px && 
					Mouse.Px <= WD_Size_W/2 + BtnSize_X && 
					Mouse.Trg & _lMOUSE)) {
...
{
	// Left, Blue & Yellow
	if ((Gs_GetKEY(kKEY_A) || Gs_GetKEYtrg(kKEYLEFT) || 
		(((Mouse.Trg & _lMOUSE) || (Mouse.WClk & _lMOUSE)) && 
		(Mouse.Px < WD_Size_W / 2))) && !Reset) {
		if (BlkVl[0].Rdm == 2 || BlkVl[0].Rdm == 4) {
			if (!Fever)
				iDarumaHead = 1;
			Gs_PlaySE(SND_Hit);
			GameAni = true; Reset = true; AniMove = 1;
			BlkReset = true; iBlkHit = 1;
			Blk[0].bPosition.x = WD_Size_W / 2;
			Blk[0].bSpdX = 150.0f;
			Blk[0].bSpdX_Down = 0.9f;
	...
	//Right, Red & Green
	if ((Gs_GetKEY(kKEY_A) || Gs_GetKEYtrg(kKEYRIGHT) || 
		(((Mouse.Trg & _lMOUSE) || (Mouse.WClk & _lMOUSE)) && 
		(Mouse.Px > WD_Size_W / 2))) && !Reset) {
...
void SystemCheckGame(void)
{
	...
	if (GameSystemNum[0] && GameSystemNum[1] && 
		GameSystemNum[2] && GameSystemNum[3])
	{
